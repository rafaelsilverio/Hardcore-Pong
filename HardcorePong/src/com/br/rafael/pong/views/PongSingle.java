	...
	public boolean onTouchEvent(MotionEvent event){
		...
		if(telaPressionada){
			...
			if(threadJogo.getGameState().getAcaoSubir().verificaColisao(event.getX(), event.getY())){
				threadJogo.getGameState().getPlayer1().setMovimentoAtual(Jogador.MOVIMENTO_ACIMA);
				...
				threadJogo.getGameState().getPlayer1().setMovimentoAtual(Jogador.MOVIMENTO_ABAIXO);
				threadJogo.getGameState().getAcaoDescer().setDesaparecer(true);
			}
			else
				threadJogo.getGameState().getPlayer1().setMovimentoAtual(Jogador.MOVIMENTO_NULO);
		}
